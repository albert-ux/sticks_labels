...
export class CreateOrdenComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.ordenForm.get('clienteId')?.valueChanges.subscribe(clienteIds => {
      console.log('Cliente IDs seleccionados:', clienteIds);
  ...
  setCliente(event: any, index: number) {
    const clienteId = event.target.value;
    const cliente = this.clientes.find(c => c.id == clienteId);
    if (cliente && cliente.representantes.length > 1) {
      // Aquí debes cargar los representantes del cliente seleccionado en el formArray de participantes.
      // Por simplicidad, se asume que cada cliente tiene una propiedad "representantes" que es un array.
      const representantesControl = this.participantes.at(index).get('representanteId');
      representantesControl?.setValue(cliente.representantes[0].id);  // O cómo quieras gestionarlo
    }
  }
  ...
  guardarOrden(): void {
    ...
    console.log('Orden a guardar:', this.ordenForm.value);
    this.backendService.createOrden(this.ordenForm.value).then(() => {
      this.alertService.showAlert('success', 'Inscripción creada', 'La inscripción se ha creado correctamente');
