...
@Component({
  selector: 'app-docs-asesor',
  templateUrl: './docs-asesor.component.html'
})
export class DocsAsesorComponent implements OnInit {
  ...
  ngOnInit(): void {
    if (this.id) {
      this.backendService.getFilesOfAsesor(this.id).subscribe(files => {
        this.files = files;
      });
    }
  }
  ...
  async uploadFile(): Promise<void> {
    if (this.selectedFile && this.customFileName) {
      const filePath = `asesorFiles/${new Date().getTime()}_${this.customFileName}`;  // Usar el nombre personalizado aquí
      ...
      if (result.status === 'completed') {
        ...
        this.backendService.addFileToAsesor(this.id, fileData).then(() => {
  ...
  eliminarArchivo(fileId: string, filePath: string): void {
    if (window.confirm('¿Estás seguro de que deseas eliminar este archivo?')) {
      ...
      deleteChain.then(() => {
        return this.backendService.deleteFileOfAsesor(fileId);
      })
        .then(() => {
